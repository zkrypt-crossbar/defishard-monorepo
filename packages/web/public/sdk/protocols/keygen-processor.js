import { KeygenSession, Message } from '../../pkg/dkls_wasm_ll.js';
    ...
    async initialize(oldKeyshare) {
        try {
            if (oldKeyshare) {
                ...
                this.session = KeygenSession.initKeyRotation(oldKeyshare, undefined, // seed
                this.keygenConfig.distributed);
            }
            else {
                // Regular DKG initialization
                this.session = new KeygenSession(this.config.groupInfo.totalParties, this.config.groupInfo.threshold, this.config.partyIndex, this.hexToBytes(this.config.groupId), undefined, // seed
                this.keygenConfig.distributed);
            }
            this.currentRound = -1;
        }
    ...
    async saveKeyshare(keyShare) {
        try {
            ...
            const keyshareData = {
                ...
                publicKey: Array.from(keyShare.publicKey).map(b => b.toString(16).padStart(2, '0')).join(''), // Convert to hex string
